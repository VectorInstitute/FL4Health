...
CLIENT_DATA_BASE_PATH="flamby_datasets/fed_ixi"
...
mkdir -p log_distributed
SERVER_OUT_LOGS="log_distributed/server_log_${SERVER_JOB_HASH}.out"
SERVER_ERROR_LOGS="log_distributed/server_log_${SERVER_JOB_HASH}.err"
...
SBATCH_COMMAND="--job-name=${SERVER_JOB_NAME} --output=${SERVER_OUT_LOGS} --error=${SERVER_ERROR_LOGS} \
  fed_ixi/run_server.slrm ${SERVER_PORT} ${SERVER_CONFIG_PATH} ${SERVER_LOG_DIR} ${VENV_PATH} \
  ${SERVER_JOB_HASH}"
...
do
  ...
  CLIENT_OUT_LOGS="log_distributed/client_log_${CLIENT_JOB_HASH}.out"
  CLIENT_ERROR_LOGS="log_distributed/client_log_${CLIENT_JOB_HASH}.err"

  SBATCH_COMMAND="--job-name=${CLIENT_JOB_NAME} --output=${CLIENT_OUT_LOGS} --error=${CLIENT_ERROR_LOGS} \
    fed_ixi/run_client.slrm ${SERVER_ADDRESS} ${CLIENT_DATA_PATH} ${CLIENT_LOG_DIR} ${VENV_PATH} \
    ${CLIENT_JOB_HASH}"
