...
def compute_mult_and_computeeigen_info(
    numpy_mat: np.ndarray, torch_matrix: torch.Tensor
) -> Tuple[Any, ...]:
    mat = np.matmul(numpy_mat, torch_matrix)
    return linalg.eig(mat)


eigen_vals, eigen_vectors = compute_mult_and_computeeigen_info(
    np_matrix, torch_matrix
)
