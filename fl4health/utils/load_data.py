...
def load_cifar10_data(
    ...
    batch_size: int,
    sampler: Optional[LabelBasedSampler] = None,
    additional_transform: Union[None, Callable] = None,
    dataset_converter: Optional[DatasetConverter] = None,
) -> Tuple[DataLoader, DataLoader, Dict[str, int]]:
    """Load CIFAR-10 (training and validation set)."""
    log(INFO, f"Data directory: {str(data_dir)}")
    transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), additional_transform]
    )
    train_ds: BaseDataset = Cifar10Dataset(data_dir, train=True, transform=transform)
    val_ds: BaseDataset = Cifar10Dataset(data_dir, train=False, transform=transform)
