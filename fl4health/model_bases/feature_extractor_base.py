...
class FeatureExtractorModel:
    def __init__(
        self, model: nn.Module, output_layers: Sequence[str], flatten_features: Optional[Sequence[bool]]
    ) -> None:
        self.model = model
    ...
    def forward(self, input: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        for i, layer in enumerate(self.selected_out):
            features[layer] = (
                self.flatten(self.selected_out[layer]) if self.flatten_features[layer] else self.selected_out[layer]
            )
