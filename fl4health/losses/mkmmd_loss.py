...
class MkMmdLoss(torch.nn.Module):
    ...
    def optimize_betas(self, X: torch.Tensor, Y: torch.Tensor, lambda_m: float) -> torch.Tensor:
        ...
        if self.minimize_type_two_error:
            ...
            # If we're trying to maximize the type II error, then we are trying to maximize a concave function over a
            # convex polygon of beta values. So the maximum is found at one of the vertices
            return self.get_best_vertex_for_objective_function(hat_d_per_kernel, regularized_Q_k)
