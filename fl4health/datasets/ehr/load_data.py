...
class CachedDataset(Dataset):  # Dataset
    def __init__(self, hospital_id, unitstays, data_path):
    ...
    for i in range(len(args.hospital_id)):
        ...
        if len(train_dataset[i]) % args.batch_size == 1:
            train_loaders.append(
                DataLoader(
                    train_dataset[i], sampler=train_sampler, batch_size=args.batch_size, num_workers=8, drop_last=True
                )
            )
        else:
            train_loaders.append(
                DataLoader(
                    train_dataset[i], sampler=train_sampler, batch_size=args.batch_size, num_workers=8, drop_last=False
                )
            )
        ...
        if len(valid_dataset[i]) % args.batch_size == 1:
            valid_loaders.append(
                DataLoader(
                    valid_dataset[i], sampler=valid_sampler, batch_size=args.batch_size, num_workers=8, drop_last=True
                )
            )
        else:
            valid_loaders.append(
                DataLoader(
                    valid_dataset[i], sampler=valid_sampler, batch_size=args.batch_size, num_workers=8, drop_last=False
                )
            )
        ...
        if len(test_dataset[i]) % args.batch_size == 1:
            ...
            test_loaders.append(
                DataLoader(test_dataset[i], batch_size=args.batch_size, num_workers=8, drop_last=False)
            )
