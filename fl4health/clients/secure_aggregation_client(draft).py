    ...
    def get_properties(self, config: Config) -> Dict[str, Scalar]:
        ...
        match config["event_name"]:
            ...
            case Event.SHARE_KEYS.value:
                ...
                # generate self mask seed (which is not returned here, because it is not used at this step)
            ...
            case Event.MASKED_INPUT_COLLECTION.value:
                ...
                assert len(received) >= self.crypto.reconstruction_threshold
                self.crypto.register_shamir_shares(shamir_shares=received)  # receive Shamir shares from other clients

                # masked vector is sent to server via the fit() method above.
                self.debugger(self.crypto.client_integer, received.keys())
                self.debugger(self.crypto.bob_shamir_secrets)
