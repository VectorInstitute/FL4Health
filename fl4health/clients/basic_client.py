...
class BasicClient(NumPyClient):
    ...
    def _move_target_data_to_device(self, data: TorchTargetType) -> TorchTargetType:
        """
        Move target data to self.device

        Args:
            data (TorchTargetType): target data to calculate loss and metrics with

        Returns:
            TorchTargetType: target data but now its on self.device

        Raises:
            TypeError: raised if target data is not of type torch.Tensor, List[torch.Tensor] or Tuple[torch.Tensor]
        """
        ...
        if isinstance(data, torch.Tensor):
            ...
            raise TypeError("target data must be of type torch.Tensor, List[torch.Tensor] or Tuple[torch.Tensor]")
