...
def infer_label_dim(tensor1: torch.Tensor, tensor2: torch.Tensor) -> int:
    ...
    # An example is tensor_1.shape = (5, 5, 3) and tensor_2.shape = (5, 3).
...
def align_pred_and_target_shapes(
    ...
    if preds.ndim > targets.ndim:
        ...
        # Determine label dimension. Preds is the first/reference tensor because its shape is larger
    ...
    if preds.ndim < targets.ndim:
        ...
        # Determine label dimension. Targets is the first/reference tensor because its shape is larger
    ...
    # Determine label dimension. Because their shapes are the same length, order doesn't matter
    label_dim = infer_label_dim(preds, targets) if label_dim is None else label_dim
