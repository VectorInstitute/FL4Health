...
def infer_label_dim(tensor1: torch.Tensor, tensor2: torch.Tensor) -> int:
    ...
    for idx1 in range(tensor1.ndim):
        if idx2 >= tensor2.ndim:
            # Just in case its the last label we need to avoid indexing error
    ...
    # candidate label when technically it is ambiguous. An example is
...
def map_label_index_tensor_to_one_hot(
    ...
    """
    Maps the provided ``label_index_tensor``, which has label indices at the provided ``label_dim``. This function
    uses the shape provided by ``target_shape`` to expand the label indices into one-hot encoded vectors in that
    dimension according the the size of the target dimension at ``label_dim`` in ``target_shape``. For example,
    if label_index_tensor has shape (64, 10, 10, 1), label_dim = 3, and target_shape = (64, 10, 10, 4), then the new
    shape should be (64, 10, 10, 4) with [i, j, k, :] being a one-hot vector of length 4.
...
def align_pred_and_target_shapes(
    ...
    hold in binary classification settings where continuos values might be used to indicate the positive label by
