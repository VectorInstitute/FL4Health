...
# TODO rename file to poisson_binomial.py, move tests to test/


def scaler_poisson_binomial(x: float, N: int, radius: float, theta: float) -> int:
    """
    Privacy mechanism which encodes scalar data x as a binomial random variable.
    ...
    Returns:
        int: A random sample taken from the binomial distribution with success probability
    ...
    # rescaling
    p = theta * x / radius + 0.5

    return Binomial(N, p).sample().int().item()
...
def poisson_binomial_client(
    ...
    Args:
        query_vector (List[float]): Client vector.
        radius (float): Bound on 2-norm of query_vector.
        tight_frame (List[List[float]]): Matrix to compute Kashin representation.
        theta (float): A parameter within the real interval [0, 1/4].
    ...
    scaled_radius = radius * K / math.sqrt(dim)

    privatization = torch.zeros(dim)
    for j in range(dim):
        privatization[j] = scaler_poisson_binomial(kashin_vector[j].item(), modulus - 1, scaled_radius, theta)

    return list(privatization)


def poisson_binomial_server(
...
if __name__ == "__main__":
    # # scaler_poisson_binomial test cases
    # x, N, radius, theta = 0, 20, 5, 0.1
    # out = [scaler_poisson_binomial(x, N, radius, theta) for _ in range(100)]
    # print(out)

    # Poissonbinomial_mechanism test cases
