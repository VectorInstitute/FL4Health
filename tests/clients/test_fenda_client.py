...
def test_getting_parameters(get_fenda_client: FendaClient) -> None:  # noqa
    ...
    new_params_local = [val.cpu().numpy() for _, val in fenda_client.model.first_module.state_dict().items()]
    ...
    new_params_global = [val.cpu().numpy() for _, val in fenda_client.model.second_module.state_dict().items()]
