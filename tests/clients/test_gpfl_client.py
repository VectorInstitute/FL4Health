...
def test_gpfl_setup_client(get_gpfl_client: GpflClient) -> None:  # noqa
    ...
    with patch.object(basic_client, "setup_client") as mock_super_setup_client:
        ...
        assert torch.equal(client.sample_per_class, torch.Tensor([0.5, 0.5]))
...
def test_compute_conditional_inputs(get_gpfl_client: GpflClient) -> None:  # noqa
    ...
    manual_sample_per_class = torch.Tensor([0.3, 0.4, 0.3])
    ...
    client.sample_per_class = manual_sample_per_class
    # Manually compute the conditional inputs
    manual_g = (manual_embedding[0, :] + manual_embedding[1, :] + manual_embedding[2, :]) / 3
    manual_p = (
        manual_embedding[0, :] * manual_sample_per_class[0]
        + manual_embedding[1, :] * manual_sample_per_class[1]
        + manual_embedding[2, :] * manual_sample_per_class[2]
    )
    ...
    assert torch.allclose(client.generic_conditional_input, manual_g, atol=1e-6), (
...
def test_transform_input_and_predict(get_gpfl_client: GpflClient) -> None:  # noqa
    ...
    generic_conditional_input = torch.ones(256)
    personalized_conditional_input = torch.ones(256)
    client.generic_conditional_input = generic_conditional_input
    ...
    assert {"input", "generic_conditional_input", "personalized_conditional_input"} == set(transformed_input.keys())
    assert torch.equal(transformed_input["generic_conditional_input"], generic_conditional_input)
...
def test_gpfl_training_loss(get_gpfl_client: GpflClient) -> None:  # noqa
    ...
    assert torch.allclose(
        ...
        (
            ...
            + loss.additional_losses["magnitude_level_loss"]
        ),
    )
