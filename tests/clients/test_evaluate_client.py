...
def test_evaluating_identical_global_and_local_models(get_evaluation_client: EvaluateClient) -> None:  # noqa
    ...
    assert pytest.approx(metrics["local_eval_meter - predictions - accuracy"], abs=0.0001) == 0.0
    assert pytest.approx(metrics["global_eval_meter - predictions - accuracy"], abs=0.0001) == 0.0
