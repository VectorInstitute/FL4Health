...
def test_multi_dice_metric_with_threshold() -> None:
    ...
    hd = MultiClassDice(name="DICE", label_dim=2, batch_dim=None, threshold=2, zero_division=None)
    ...
    hd.update(logits, targets)
    result = hd.compute()
    # Should be the same since all added piece have zero division and are ignored
    assert result["DICE"] == approx((1.0 / 3.0) * (4.0 / 8.0 + 10 / 13.0 + 2.0 / 8.0))

    # Move to zero division = 1.0 such that each of these examples adds 1.0 to the DICE
    hd.zero_division = 1.0
    result = hd.compute()
    assert result["DICE"] == approx((1.0 / 3.0) * (4.0 / 8.0 + 10 / 13.0 + 2.0 / 8.0))

    hd.zero_division = None
...
def test_multi_dice_metric_ignore_background() -> None:
    ...
    hd = MultiClassDice(name="DICE", label_dim=2, batch_dim=None, threshold=2, ignore_background=2, zero_division=None)
    hd.update(logits, targets)
    result = hd.compute()
    # TP = [1, 2, 0], TN = [3, 3, 2], FP = [1, 1, 1], FN = [1, 0, 3].
...
def test_continuous_multi_dice_metric() -> None:
    ...
    hd = MultiClassDice(name="DICE", label_dim=2, batch_dim=None, zero_division=None)
    hd.update(logits, targets)
    result = hd.compute()
...
def test_more_spatial_dimensions() -> None:
    metric = MultiClassDice(name="DICE", label_dim=1, batch_dim=None)
    ...
    metric = MultiClassDice(name="DICE", label_dim=0, zero_division=1.0, threshold=0.1, batch_dim=None)
    all_ones_targets = torch.ones((10, 2, 10, 10))
    ones_and_zeros_logits = torch.ones((10, 2, 10, 10))
    # Set entries in the second channel to zero for both target and preds
    ones_and_zeros_logits[:, 1, :, :] = 0
