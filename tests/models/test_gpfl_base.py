...
def test_gpfl_model_gets_correct_layers() -> None:
    ...
    base_layers = [
        layer_name for layer_name in gpfl_model.state_dict() if layer_name.startswith("main_module.base_module.")
    ]
    head_layers = [
        layer_name for layer_name in gpfl_model.state_dict() if layer_name.startswith("main_module.head_module.")
    ]
...
def test_gpfl_model_forward_steps() -> None:
    ...
    generic_conditional_input = torch.randn(16 * 4 * 4)
