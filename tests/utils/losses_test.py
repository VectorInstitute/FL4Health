...
from fl4health.utils.losses import LossAccumulationMeter, LossAverageMeter, Losses, TrainLosses
...
def test_train_loss_average_meter() -> None:
    losses = [
        TrainLosses(
            checkpoint=torch.tensor(1.5837, dtype=torch.float),
...
def test_train_loss_accumulation_meter() -> None:
    losses = [
        TrainLosses(
            checkpoint=torch.tensor(1.5837, dtype=torch.float),
            backward=torch.tensor(4.3443, dtype=torch.float),
...
def test_losses_with_multiple_backward() -> None:
    ...
    losses = TrainLosses(checkpoint=checkpoint, backward=backward_losses_dict)
