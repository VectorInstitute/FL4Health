...
SERVER_PORT=8081
SERVER_CONFIG_PATH="examples/basic_femnist_example/config.yaml"
LOG_DIR="examples/basic_femnist_example/log"
VENV_PATH="fl4health_ddgm"

num_clients=500
CLIENT_BATCH_SIZE=25
NUM_SLURM=20 # num of slurm jobs
...
SERVER_JOB_ID=$( sbatch --parsable \
    ...
    examples/basic_femnist_example/batch_exec/run_server.slrm \
      "${SERVER_PORT}" \
      "${SERVER_CONFIG_PATH}" \
      "${LOG_DIR}" )
...
for i in $(seq 1 "$CLIENT_BATCH_SIZE" "$num_clients"); do
  ...
  jid=$( sbatch --parsable \
      ...
      examples/basic_femnist_example/batch_exec/run_client.slrm \
        "${SERVER_ADDRESS}" \
        "${LOG_DIR}" \
        "${i}" \
        "${CLIENT_BATCH_SIZE}" \
