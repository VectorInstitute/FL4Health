    ...
    def __init__(
        ...
        if train:
            augmentations = albumentations.Compose(
                [   albumentations.ToFloat(always_apply=True, max_value=256),
                    ...
                    albumentations.Normalize(mean=mean, std=std, always_apply=True, max_pixel_value=1),
                ]            
            )
        else:
            augmentations = albumentations.Compose(
                [
                    albumentations.ToFloat(max_value=256, always_apply=True),
                    albumentations.Resize(height=232, width=232, always_apply=True),
