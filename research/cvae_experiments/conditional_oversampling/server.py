...
def main(config: Dict[str, Any]) -> None:
    ...
    fit_config_fn = partial(
        fit_config, config["local_epochs"], config["batch_size"], config["latent_dim"], config["checkpoint_path"]
    )
    ...
    # If we pass the condition vectors during parameter initialization,
    #  then all the clients will have the condition matrix.
    # To avoid that we override the configure_fit() in strategy to send individual vectors to each client at round 1.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/research/cvae_experiments/conditional_oversampling/config.yaml",
    )
