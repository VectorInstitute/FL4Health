...
def main(config: dict[str, Any], server_address: str, checkpoint_stub: str, run_name: str) -> None:
    ...
    if int(config["n_clients"]) == 6:
        # delirium
        #  correct size: 8093, but for heterogeneity experiment use 300
        client_model = delirium_model(input_dim=8093, output_dim=1)
    else:
        # mortality
        client_model = mortality_model(input_dim=35, output_dim=1)
