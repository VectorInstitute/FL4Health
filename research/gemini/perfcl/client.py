...
class GeminiPerFclClient(FendaClient):
    ...
    def get_model(self, config: Config) -> nn.Module:
        if self.learning_task == "mortality":
            model: nn.Module = GeminiPerFclModel(input_dim=35, output_dim=1).to(self.device)
        else:
            model: nn.Module = DeliriumPerFclModel(input_dim=8093, output_dim=1).to(self.device)
        return model
