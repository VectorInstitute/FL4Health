...
from fl4health.metrics.metrics import AccumulationMeter, Metric
...
def main(
    ...
    # Load training and validation data from all the hospitals.
    if learning_task == "mortality":
        model: nn.Module = mortality_model(input_dim=35, output_dim=1).to(device)
        train_loader, val_loader, num_examples = load_train_mortality(data_path, batch_size, None)
    else:
        model: nn.Module = delirium_model(input_dim=8093, output_dim=1).to(device)
        train_loader, val_loader, num_examples = load_train_delirium(data_path, batch_size, None)

    checkpoint_dir = os.path.join(checkpoint_stub, run_name)
    checkpoint_name = "centralized_best_model.pkl"
    log(INFO, f"logging address: {checkpoint_dir}")
    ...
    for epoch in range(num_epochs):
        ...
        # epoch_train_loss = training_loss_sum / len(train_loader)
    ...
    log(INFO, f"Best Loss seen by the model: \n{checkpointer.best_metric}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_epochs", action="store", type=int, default=60, help="Number of epochs")
    parser.add_argument(
        "--artifact_dir",
        action="store",
        type=str,
