...
def get_preprocessed_data(
    ...
    train_data = np.load(f"{dataset_dir}/client_{client_num}_train_data.npy")
    train_targets = np.load(f"{dataset_dir}/client_{client_num}_train_targets.npy")
    training_set = TensorDataset(train_data, train_targets, transform=transform, target_transform=None)

    validation_data = np.load(f"{dataset_dir}/client_{client_num}_validation_data.npy")
    validation_targets = np.load(f"{dataset_dir}/client_{client_num}_validation_targets.npy")
...
def get_test_preprocessed_data(
    ...
    data = np.load(f"{dataset_dir}/client_{client_num}_test_data.npy")
    targets = np.load(f"{dataset_dir}/client_{client_num}_test_targets.npy")
...
def save_preprocessed_data(save_dataset_dir: Path, partitioned_datasets: List[TensorDataset], mode: str) -> None:
    for client in range(len(partitioned_datasets)):
        ...
        np.save(f"{save_dataset_dir}/client_{client}_{mode}_data.npy", save_data.numpy())
        if save_targets is not None:
            np.save(f"{save_dataset_dir}/client_{client}_{mode}_targets.npy", save_targets.numpy())
...
if __name__ == "__main__":
    ...
    save_preprocessed_data(Path(args.save_dataset_dir), train_partitioned_datasets, "train")
    save_preprocessed_data(Path(args.save_dataset_dir), validation_partitioned_datasets, "validation")
    save_preprocessed_data(Path(args.save_dataset_dir), test_partitioned_datasets, "test")
