...
def main() -> None:
    ...
    parser.add_argument('-d', type=int, required=True, help='The nnUNet dataset ID integer for the fine-tuning dataset')
    parser.add_argument('--pt_d', type=int, required=False, help='The dataset ID integer of the pretraining dataset. Can be omitted if pt_fingerprint and pt_plans flags are both included')
    parser.add_argument('--pt_fingerprint', type=str, required=False, help='Path to the dataset_fingerprint json for the dataset used in pretraining')
    parser.add_argument('--pt_plans', type=str, required=False, help='Path to the experiment plans used for the pretrained model')
    parser.add_argument('--pt_checkpoint', type=str, required=True, help='Path to the model checkpoint for the pretrained model to use as a starting point')
    parser.add_argument('-c', type=str, required=True, help="Configuration that should be trained. Eg. '2d', '3d_fullres'")
    parser.add_argument('-f', type=int, required=True, help='Fold of the 5-fold cross-validation. Should be an inte between 0 and 4')
    ...
    # Note can possibly extend this to copy dataset json of pretrained dataset and just change number of samples to create new dataset json for finetuning set. For now assume it exists in raw data folder
