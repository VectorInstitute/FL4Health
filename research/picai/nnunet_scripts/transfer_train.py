...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="The dataset ID integer of the pretraining dataset. Can be omitted if pt_fingerprint and pt_plans flags are both included",
    )
    parser.add_argument(
        "--pt_fingerprint",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Use this flag to specify a custom trainer. Some alternatives are already provided by nnUNet. Eg. 'nnUNetTrainer_5epochs'",
    )
    parser.add_argument(
        "--skip_preprocessing",
        type=bool,
        required=False,
        default=False,
        help="Whether or not to skip the preprocessing step. Only set this to True if you have already run this command with preprocessing and only want to redo the fine-tuning",
    )
