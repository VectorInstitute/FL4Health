...
from research.flamby.fed_ixi.moon_mkmmd.moon_model import FedIxiMoonModel
from research.flamby.flamby_data_utils import construct_fed_ixi_train_val_datasets


class FedIxiMoonClient(MoonClient):
    def __init__(
        self,
        data_path: Path,
    ...
    def get_model(self, config: Config) -> nn.Module:
        model: nn.Module = FedIxiMoonModel().to(self.device)
        return model
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="Path to the preprocessed FedIXI Dataset (ex. path/to/fedisic2019)",
        required=True,
    )
    parser.add_argument(
        "--run_name",
