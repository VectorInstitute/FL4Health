...
import torch
from flamby.datasets.fed_ixi import BATCH_SIZE, NUM_CLIENTS, FedIXITiny
...
def main(
    ...
    for client_number in range(NUM_CLIENTS):
        client_test_dataset = FedIXITiny(center=client_number, train=False, pooled=False, data_path=dataset_dir)
        ...
        for run_folder_dir in all_run_folder_dir:
            if eval_local_models:
                ...
                local_run_metric = evaluate_fed_ixi_model(local_model, test_loader, metrics, device, is_apfl)
                log(
                    INFO,
            ...
            if eval_global_model:
                ...
                server_run_metric = evaluate_fed_ixi_model(server_model, test_loader, metrics, device, is_apfl)
                log(
                    INFO,
                    f"Client Number {client_number}, Run folder: {run_folder_dir}: "
    ...
    if eval_global_model:
        ...
        for run_folder_dir in all_run_folder_dir:
            ...
            run_metric = evaluate_fed_ixi_model(model, pooled_test_loader, metrics, device, is_apfl)
