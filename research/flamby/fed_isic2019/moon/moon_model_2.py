...
class ProjectionModule(nn.Module):
    """Moon projection module"""

    def __init__(self, stack_output_dimension: int, projection_output_dimension: int):
        super().__init__()
        self.fc1 = nn.Linear(stack_output_dimension, projection_output_dimension)
        self.fc2 = nn.Linear(projection_output_dimension, projection_output_dimension)
...
class FedIsic2019MoonModel(MoonModel):
    def __init__(self, frozen_blocks: Optional[int] = 13, turn_off_bn_tracking: bool = False) -> None:
        ...
        projection_modeule = ProjectionModule(1280, 256)
        head_module = HeadClassifier(256)

        super().__init__(base_module, head_module, projection_modeule)
