...
class Fed_TCGA_BRCA_Client(SecureAggregationClient):
    def __init__(
        self,
        data_path: Path,
        ...
        super().__init__(
            ...
            task_name='Fed-TCGA-BRCA Distributed',
        )
    ...
    def get_data_loaders(self, config: Config) -> Tuple[DataLoader, DataLoader]:
        train_dataset, validation_dataset = construct_fed_tcga_brca_train_val_datasets(
            self.client_number
        )
    ...
    def get_model(self, config: Config) -> nn.Module:
        model: nn.Module = Fed_TCGA_BRCA_LargeBaseline().to(self.device)
        return model
...
if __name__ == "__main__":
    ...
    privacy_settings = {
        ...
        'bias': config['bias'],
        # NOTE 'model_integer_range' not needed by client
    ...
    client = Fed_TCGA_BRCA_Client(
        data_path=Path(args.dataset_dir),
        metrics=[C_Index("concordance index")],
        device=DEVICE,
