...
class PersonalServer(FlServer):
    ...
    fall under this category. Each client will have its own model that is specific to its own training. Personal models
    may have shared components but the full model is specific to each client. As such, there is no sense of a GLOBAL
    model to be checkpointed on the server-side that is shared by all clients. We eliminate the possibility of
