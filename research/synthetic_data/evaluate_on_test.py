...
from research.synthetic_data.preprocess import get_test_preprocessed_data
from research.synthetic_data.utils import (
    evaluate_synthetic_data_model,
    get_all_run_folders,
    get_metric_avg_std,
    ...
    for client_number in range(NUM_CLIENTS):
        ...
        for run_folder_dir in all_run_folder_dir:
            if eval_best_pre_aggregation_local_models:
                ...
                local_run_metric = evaluate_synthetic_data_model(local_model, test_loader, metrics, device, is_apfl)
                log(
                    INFO,
            ...
            if eval_best_global_model:
                ...
                server_run_metric = evaluate_synthetic_data_model(server_model, test_loader, metrics, device, is_apfl)
                log(
                    INFO,
                    f"Client Number {client_number}, Run folder: {run_folder_dir}: "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="Path to the preprocessed Synthetic Dataset (ex. path/to/synthetic_data)",
        required=True,
    )
    parser.add_argument(
        "--eval_write_path",
